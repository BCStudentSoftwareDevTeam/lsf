...
def getPositionInfo(orgCode):
    ...
    for deptCode in givenOrgCode:
        celtsLabor = (LaborStatusForm.select(LaborStatusForm.studentSupervisee_id, LaborStatusForm.termCode_id,  LaborStatusForm.jobType, LaborStatusForm.WLS, LaborStatusForm.POSN_TITLE)
                                     .where(LaborStatusForm.department_id == deptCode))
        
        for laborForm in celtsLabor:
